...
// 2. Score discriminates by >25 pts (the brief's threshold)
const discriminates = Math.abs(spread) > 25;
line(
  discriminates,
  "score DISCRIMINATES (>25)",
  `careful ${f(A.score)} − bodycount ${f(B.score)} = SPREAD ${f(spread)}  ${discriminates ? "" : "→ strategy choice barely matters"}`
);

// 3. Funding a project actually moves attitude (Δattitude meaningfully positive for A)
...
line(
  aDelta - bDelta > 5,
  "COIN moves attitude",
  `careful Δatt ${f(aDelta)} vs bodycount Δatt ${f(bDelta)}  (careful projects complete ${f(projComplete, 1)})`
);
...
// 5. CERP is a two-way economy (not strictly one-way drain)
...
// 7. projects actually complete under the secure-build order (was 0 — all sabotaged)
const projCompleteMean = mean(careful.map((r) => r.projComplete));
line(
  projCompleteMean >= 1,
  "projects COMPLETE (secured)",
  `careful completes ${f(projCompleteMean, 1)} projects/tour (was 0 — all sabotaged)`
);

// 8. directives issued on a cadence (more than the 2 initial)
...
console.log(
  `\nINTERPRETATION: ${
    discriminates && cerpRose && dirLive
