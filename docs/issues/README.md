...
| [007](007-sim-level-terrain-ecology.md) | Terrain ecology is render-deep, not sim-deep (aspect, terraces, qalats, hydrology) | Low (fidelity) | 🟡 **Aspect-vegetation implemented+proven; can't ship active (3-strength balance sweep proves it)** — forest-faces-north 59→73%, but a same-seed A/B at 0.16/0.05/0.025 gives KIA 1.83/1.08/1.50 (NON-monotonic) with intermittent strandings: it perturbs combat+movement CHAOTICALLY (changes which terrain firefights happen on), and a sim-deep aspect can't be neutralized without making forest≡scrub (render-deep). PRESERVED behind an off-by-default flag (`ITM_ASPECT`, byte-identical at 0) + probe + 3 data points for an owner-approved balance-compensation pass. 🟡 **Footpaths shipped 2026-06-09** — switchback foot-trails up the spurs + every path centerline captured & drawn as scaled lines (trail cells 492→760, paths/seed ~6→22-32). Deeper ecology (aspect/terraces/qalats/hydrology) still open. See `docs/progress/2026-06-09-terrain-realism/` |
