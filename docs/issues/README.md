...
| [031](031-patrol-straggler-cohesion-floor-tension.md) | Patrol straggler cohesion: `follower-strand` reports a 70–108 m worst-follower gap on worst-opposite **far** villages (always the heaviest man) — looks like "not sticking together" | Low (texture) | ✅ **WORKING AS INTENDED + refuted fix recorded 2026-06-27** — the *authoritative* `squad-arrival` already shows **squadCoh 100 %, maxStrag 33 m**: the squad closes up at the objective; the 70–108 m is a first-tick instant sample of a realistic **climb-stretch** that re-forms at the halt (`maxWedge 0.0 s`, nobody stuck). The obvious fix (reorder the never-freeze floor so the navigator eases below it to wait) tightens the easy seeds (foxtrot/kilo/oscar 70–76→23–54 m) but **regresses india-9** (lead misses the window — the slow-failure of 009/RC#2) at every creep floor; **reverted**, combat.ts byte-identical. The floor is load-bearing; do not re-attempt. |
