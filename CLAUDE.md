...
- **Standing checks green** (run as background tasks so work continues): `npx tsc --noEmit` · `npm run build` · `npm run lint` (flat-config eslint, NOT `next lint`) · `npx tsx scripts/smoke.ts` (no-NaN + serialize round-trip + material-hash → `SMOKE OK`) · `npx tsx scripts/balance.ts` (STALL CHECK gate; **casualties are a DIAGNOSTIC, not a target — there is NO WIA band to defend**). For any **sim / AI / COIN / balance** change, run the **win-condition GATE** `npx tsx scripts/campaign-loop.ts` (does playing COIN well beat playing it badly? exits non-zero if COIN goes inert). Its principle + exit-code are in; its **practical config is NOT yet calibrated** — the 3-seed default can run ~40 min (one heavy body-count seed is 9–15 min) and the fast 1-seed/8-day config under-discriminates, so don't trust a quick run as authoritative yet (issue 030 · `docs/wiki/Harnesses.md`). No jest/vitest/`npm test` — verify via `npx tsx scripts/<name>.ts`. Prefer exhaustive `Record<Enum,…>` tables so the compiler enforces completeness.
