...
10. **Work on `main` by default — create a branch or worktree only when the user asks for one** (or when parallel agents would otherwise collide on the same files; then partition one-writer-per-file). **Commit as you go in small, precise, atomic checkpoints** — this is trunk-based development on a shared `main` with other agents committing in parallel, so frequent commits keep the working tree manageable and every change traceable. Stage only the files *you* changed (`git add <path>…`, never a blind `git add -A`/`git add .`) so you never sweep up another agent's in-flight work; keep each commit green (standing checks pass) and self-contained — **one logical change per commit**, not an end-of-session dump. **Expect other agents' commits to land between your last one and your next — that's normal, not a conflict to fight**; rebase/pull onto the latest `main` and never rewrite shared history. Pushing to the remote stays the owner's call unless asked — the owner owns the moment work leaves the machine.
...
- **PUBLISH every HTML report/explainer into the shipped archive — `public/manual/archive/` — never leave it stranded in `docs/`.** `public/` is served at deploy; `docs/` is not, so a report that lives only under `docs/progress/` is invisible to players. The archive is part of the product: it sits in the Field Manual and tells the chronological story of how the game was built (transparency is a feature here). The flow, every time you produce a report: (1) keep the raw record in `docs/progress/<date-topic>/` as always (the engineering source of truth — **never deleted**); (2) **copy** the polished `report.html` + only the images it references into `public/manual/archive/reports/<date-slug>/` (rewrite any `../` or `.md` cross-links so they resolve in the new home — reports must be self-contained or link only within the archive), and **include the shared image lightbox** — add `<script src="/manual/lightbox.js" defer></script>` before `</body>` so its screenshots open full-screen like every other user-facing doc (the lightbox is the one mechanism: it auto-finds content images, skips ones inside `<a>`/`.no-zoom`, and is wired into ALL served manual/archive/report pages — never write a per-page image-viewer); (3) **add a chapter** to `public/manual/archive/index.html` (the development-story timeline) — date, punchy title, the human-facing problem, the shape of the work, **verbatim before→after numbers**, a hero thumbnail, and a "read the full report" link; keep it chronological — **and add a card to the "All full reports" catalog** near the top of that page so every report stays reachable from one place (the rule the owner set: *all HTML reports featured in full and linked from the summary*). `docs/progress/` is the raw record; `public/manual/archive/` is the curated, served, visitor-facing copy — publish forward, don't move.
...
- **Docs are a first-class deliverable.** Update `README.md`, the relevant `docs/wiki/` page, `public/manual/` + the tutorial, and the `docs/issues/` entry (append a Resolution section with before→after numbers; update the README status table; **never delete**). Bugs you aren't fixing now → a numbered `docs/issues/` entry for a future session. **If the task produced an HTML report/explainer, it is not done until it is published into `public/manual/archive/` and added as a chapter to the archive timeline** (see §4 ▶ Delivery) — a report that ships only in `docs/` is invisible to players.
...
- **Commits** (small + atomic, *as you go* — §3 Law 10; no longer gated on an explicit ask): Conventional Commits `type(scope): subject`; evidence-heavy body (numbered root causes, before→after metrics, which harness proved it, a `tsc/build/smoke/balance green` line); linear history (`git rebase -i` / `git add -i` are blocked here). **Always include the mandatory trailer exactly — the system prompt requires it, nothing auto-adds it:**
  `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>` *(match the model named in the running session's system prompt — when this file and that prompt disagree, the prompt wins; fix this line.)*
